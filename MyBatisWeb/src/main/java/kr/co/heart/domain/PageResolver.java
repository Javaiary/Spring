...
public class PageResolver {
	...
	//SearchItem에 중복으로 정의해준 page, pageSize 삭제 (sc.getPage(), sc.getPageSiz()로 사용)
	
	private int totalCnt;			//게시물 총 개수
//	private int pageSize =10 ;		//한 페이지 당 게시물 개수
...
//				(int)Math.ceil(totalCnt/(double)pageSize);		//전체 페이지 갯수
		...
		this.beginPage= (this.sc.getPage()-1)/NAV_SIZE *NAV_SIZE +1;	// 첫 페이지 숫자 11-> 11,
	...
	public void print() {
		...
		for(int i = beginPage; i <= (totalCnt < endPage ? totalCnt : endPage) ; i++) {
			System.out.print(i + " ");
		}
